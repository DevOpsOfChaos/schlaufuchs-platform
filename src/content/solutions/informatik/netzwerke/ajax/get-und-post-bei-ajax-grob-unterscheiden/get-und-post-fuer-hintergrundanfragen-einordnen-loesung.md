---
title: Musterlösung – GET und POST grob einordnen
description: Musterlösung zur Aufgabe INF-NET-AJAX-002.
subject: informatik
section: Netzwerke
topicPath:
  - netzwerke
  - ajax
  - get-und-post-bei-ajax-grob-unterscheiden
  - get-und-post-fuer-hintergrundanfragen-einordnen-loesung
taskId: INF-NET-AJAX-002
relatedExercise: src/content/exercises/informatik/netzwerke/ajax/get-und-post-bei-ajax-grob-unterscheiden/get-und-post-fuer-hintergrundanfragen-einordnen.md
tags:
  - informatik
  - loesung
  - task-id
draft: false
---

## Aufgabe 1: Einordnen

- Vorschlagsliste holen → eher GET
- Formulardaten senden → eher POST
- Kommentare abrufen → eher GET

## Aufgabe 2: Richtung erklären

Eine abrufende Anfrage holt Informationen vom Server. Eine sendende Anfrage übermittelt Daten an den Server zur Verarbeitung.

## Aufgabe 3: Denkfehler prüfen

POST ist nicht immer besser. Die fachliche Rolle der Anfrage entscheidet, welche Methode grob sinnvoller ist.
