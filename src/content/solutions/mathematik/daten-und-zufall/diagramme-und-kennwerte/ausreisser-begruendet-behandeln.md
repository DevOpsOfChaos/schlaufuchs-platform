---
title: "Lösung – Ausreißer begründet behandeln"
description: "Musterlösung zu ausreißer begründet behandeln mit Einordnung, Abgrenzung und Gegenprobe."
subject: "mathematik"
section: "Feinstruktur Mathematik"
topicPath:
  - "daten-und-zufall"
  - "diagramme-und-kennwerte"
  - "ausreisser-begruendet-behandeln"
taskId: "v112-mathematik-ausreisser-begruendet-behandeln"
relatedExercise: "mathematik/daten-und-zufall/diagramme-und-kennwerte/ausreisser-begruendet-behandeln"
tags:
  - "mathematik"
  - "v112"
  - "feinstruktur"
  - "daten-und-zufall"
  - "ausreisser-begruendet-behandeln"
draft: false
---

## Musterlösung

Die Situation gehört zum Unterthema **Diagramme und Kennwerte**.

## Einordnung

Ein Messwert liegt deutlich außerhalb der anderen Werte. Entscheidend ist nicht der breite Fachbereich allein, sondern die konkrete Trennkante dieser Seite.

## Abgrenzung

Die typische Verwechslung lautet: Unpassende Werte werden ohne fachlichen Grund entfernt.

Sauberer ist: Ausreißer müssen begründet geprüft, nicht automatisch gelöscht werden.

## Prüfschritt

1. Kontextdaten markieren.
2. Unterthema benennen.
3. typische Verwechslung ausschließen.
4. Gegenprobe mit verändertem Beispiel formulieren.

## Gegenprobe

Wenn ein Wert, Zustand oder Begriff verändert wird, muss die Erklärung weiterhin am selben Prinzip hängen: Ausreißer müssen begründet geprüft, nicht automatisch gelöscht werden.

## Kurzfazit

Die Lösung ist gut, wenn sie die fachliche Entscheidung sichtbar macht und nicht nur ein Ergebnis behauptet.
