---
title: "Musterlösung – Relative und absolute Links passend unterscheiden"
description: "Musterlösung zur Aufgabe WEB-HTML-CORE-104."
subject: "web-development"
section: "HTML"
topicPath: ["html", "url-aufbau-und-relative-links-verstehen", "relative-und-absolute-links-passend-unterscheiden"]
taskId: "WEB-HTML-CORE-104"
relatedExercise: "src/content/exercises/web-development/html/url-aufbau-und-relative-links-verstehen/relative-und-absolute-links-passend-unterscheiden"
tags: ["web-development", "HTML", "loesung"]
draft: false
---
## Aufgabe 1: Linkart wählen

- interne Projektdatei → eher relativ
- externe Dokumentation → eher absolut
- Sprung im selben Dokument → Anker mit <code>#...</code>

## Aufgabe 2: Anker erklären

<code>#kontakt</code> verweist auf eine markierte Stelle im aktuellen Dokument.

## Aufgabe 3: Projektlogik

Relative Verlinkung ist oft robuster, wenn ein Projekt als Ganzes in ein anderes Verzeichnis oder auf einen anderen Server umzieht.
